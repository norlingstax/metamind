...
class MetamindApplication:
    ...
    def process_user_input(self, user_utterance: str, conversation_context: list[dict[str, str]]) -> dict:
        """
        ...
        try:
            # SocialMemory expects a list of strings; convert structured turns
            context_strings_for_memory = [
                f"{turn.get('speaker', 'Unknown')}: {turn.get('utterance', '')}" for turn in conversation_context
            ]
            self.social_memory.update_memory_from_interaction(
                "default_user", user_utterance, results["final_response"], context_strings_for_memory
            )
...
def run_interactive_session(app: MetamindApplication):
    ...
    current_context = [] # Stores structured turns for context passing
    ...
    while True:
        try:
            ...
            context_to_pass = current_context[-10:]
            ...
            current_context.append({"speaker": "User", "utterance": user_input})
            # agent_response is the full results dict; add only final_response
            metamind_text = agent_response.get("final_response", str(agent_response)) if isinstance(agent_response, dict) else str(agent_response)
            current_context.append({"speaker": "Metamind", "utterance": metamind_text})
...
if __name__ == "__main__":
    ...
    sample_context = [
        ...
        {"speaker": "Metamind", "utterance": "Hi! How can I help you today?"}
    ]
